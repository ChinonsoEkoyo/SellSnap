...
- **Route handler** if the caller is a public client (the product page), a third party (Paystack webhook), or another service.
...
export async function createSomething(
  ...
  try {
    ...
    // NOTE: Object.fromEntries drops duplicate keys (e.g., multi-select checkboxes).
    // If your form has array values, parse them manually with formData.getAll('key').
    const parsed = inputSchema.safeParse(Object.fromEntries(formData));
    if (!parsed.success) {
      return {
        ok: false,
        error: { code: "invalid_input", message: "Please check your entries." },
      };
    }
...
Any route that creates something paid for must be idempotent. The `Payment.gatewayReference` unique constraint is the safety net. Do not try to implement idempotency with application-level locking; the database is the source of truth.
...
- Trusting `Object.fromEntries(formData)` for multi-select arrays. It silently drops all but the final selected value. Always extract array values manually via `formData.getAll()`.
